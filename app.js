...
function initSignaturePad(){   const canvas = $('signature-canvas');   if(!canvas) return;    const ctx = canvas.getContext('2d');    const rect = () => canvas.getBoundingClientRect();    function resizeCanvas(){     const r = rect();     const ratio = window.devicePixelRatio || 1;     const width = Math.max(400, Math.floor(r.width || 400));     const height = 150;      canvas.width = width * ratio;     canvas.height = height * ratio;     canvas.style.width = width + 'px';     canvas.style.height = height + 'px';      ctx.setTransform(1, 0, 0, 1, 0, 0);     ctx.scale(ratio, ratio);     ctx.lineWidth = 2;     ctx.lineCap = 'round';     ctx.strokeStyle = '#111';   }    resizeCanvas();    let drawing = false;    function getPos(e){     const r = rect();     return {       x: e.clientX - r.left,       y: e.clientY - r.top     };   }    function start(e){     drawing = true;     const pos = getPos(e);     ctx.beginPath();     ctx.moveTo(pos.x, pos.y);   }    function move(e){     if(!drawing) return;     const pos = getPos(e);     ctx.lineTo(pos.x, pos.y);     ctx.stroke();     state.signatureDataUrl = canvas.toDataURL('image/png');   }    function end(){     if(!drawing) return;     drawing = false;     state.signatureDataUrl = canvas.toDataURL('image/png');   }    canvas.style.touchAction = 'none';    canvas.onpointerdown = function(e){     e.preventDefault();     start(e);   };    canvas.onpointermove = function(e){     e.preventDefault();     move(e);   };    canvas.onpointerup = function(e){     e.preventDefault();     end();   };    canvas.onpointercancel = function(e){     e.preventDefault();     end();   };    $('btn-clear-signature')?.addEventListener('click', () => {     ctx.clearRect(0, 0, canvas.width, canvas.height);     resizeCanvas();     state.signatureDataUrl = '';   });    window.addEventListener('resize', resizeCanvas); }
...
function bootstrap(){ bindEvents(); bindMasks(); initSignaturePad(); setupLoginScreen(); const session=getJSON(STORAGE_KEYS.session,null); if(session&&getJSON(STORAGE_KEYS.admin,null)) showApp(); else showLogin(); updateSettingsLockUI(); registerPWA(); }
