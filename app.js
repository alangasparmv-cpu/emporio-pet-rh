...
async function loadEmployeeDocuments(employeeId){ if(!state.supabase||!employeeId) return; const {data,error}=await state.supabase.from('employee_documents').select('*').eq('employee_id',employeeId).order('created_at',{ascending:false}); if(!error){ state.currentDocuments=data||[]; $('employee-documents-list').innerHTML = state.currentDocuments.length   ? state.currentDocuments.map(doc => `       <div class="list-item" style="display:flex;justify-content:space-between;align-items:center;gap:12px;">         <div>           <strong>${doc.document_type || 'Documento'}</strong><br>           <span class="muted">${doc.file_name || 'Arquivo enviado'}</span>         </div>         <div style="display:flex;gap:8px;flex-wrap:wrap;">           <a href="${doc.file_url}" target="_blank" class="secondary" style="text-decoration:none;padding:8px 12px;border-radius:6px;">Visualizar</a>         </div>       </div>     `).join('')   : '<div class="list-item">Nenhum documento enviado.</div>'; } }
